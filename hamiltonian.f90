...
allocate ( ham_J(1:2**ns,1:2**ns), ham_h(1:2**ns,1:2**ns), kp_pauli_mat(1:2**ns,1:2**ns), ord_pm(1:ns) )
...
do j = 1, ns-1
  ...
  call kron_prod_pauli_mat(ord_pm, ns, kp_pauli_mat) ;   ham_J = ham_J + kp_pauli_mat
  ord_pm(j) = 0
enddo
ham_J = -0.5d0*ham_J
...
!-----------------------------------------------------------------------------------------------------------------------------------
subroutine XY_1D(ns, h, ham) ! Returns the Hamiltonian for the 1D XY chain (the exchange energy is set to unity: J=1): 
